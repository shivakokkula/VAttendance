...
date=day+'-'+month+'-'+year+'{'+TIMESLOT+'}';
    ...
    firebaseDataRef=firebaseRef.child("DATA").child(BRANCH).child(YEAR).child(DIV);
    firebaseTheoryRef=firebaseRef.child("THEORY").child(BRANCH).child(YEAR).child(SEM).child(DIV).child(SUB).child(PROFESSOR);

    firebaseDataRef.once('value', function(snapshot)
    {
        snapshot.child("BATCH 1").forEach(function(childSnapshot)
        {
            Name[i]=childSnapshot.val()["Name"];
            Rollno[i]=childSnapshot.val()["Roll No"];
            i++;
        });
        snapshot.child("BATCH 2").forEach(function(childSnapshot)
        {
            Name[i]=childSnapshot.val()["Name"];
            Rollno[i]=childSnapshot.val()["Roll No"];
            i++;
        });
        snapshot.child("BATCH 3").forEach(function(childSnapshot)
        {
            Name[i]=childSnapshot.val()["Name"];
            Rollno[i]=childSnapshot.val()["Roll No"];
            i++;
...
{
    firebaseTheoryRef.once('value', function(snapshot)
    {
        k=0;
...
function Update()
{
    ...
    firebaseTheoryRef.once('value', function(snapshot)
    {
        ...
        {
            firebaseTheoryRef.child(date).child(i).set(hour[i]);
        }
        firebaseTheoryRef.child(date).child(i).set(totalstudents);
        i++;
        if(REMARK!="")
        {
            firebaseTheoryRef.child(date).child(i).set(REMARK);
        }        
        firebaseTheoryRef.child('Total Hours').set(total);
        j=hour.length;
        firebaseTheoryRef.child('Total Hours').child(j).set(hour.length-1);
        j++;
        firebaseTheoryRef.child('Total Hours').child(j).set("");
...
function MarkAttendance()
{
    ...
    firebaseTheoryRef.once('value', function(snapshot)
    {
        ...
        else
        {
                    ...
                    hour[i]=hours;
                    total[i]=total[i]+hours;
                    firebaseTheoryRef.child(date).set(hour);
                    i=hour.length;
                    firebaseTheoryRef.child(date).child(i).set(totalstudents);
                    i++;
                    firebaseTheoryRef.child(date).child(i).set(REMARK);        
...
function GenerateExcelSheet()
{
    ...
    exceldata[2].push("","",    BRANCH+'        '+YEAR+'       '+SEM+'       '+DIV+'        '+SUB+'        '+"Prof : "+PROFESSOR);
    ...
    firebaseTheoryRef.once('value', function(snapshot)
    {
        ...
        var filename=BRANCH+' '+YEAR+' '+SEM+' '+DIV+' '+SUB+' '+'Prof :'+' '+PROFESSOR;
    ...
    document.getElementById('heading2').innerHTML="&nbsp"+"&nbsp"+"&nbsp"+"&nbsp"+"&nbsp"+"&nbsp"+"&nbsp"+"&nbsp"+"&nbsp"+"&nbsp"+"&nbsp"+"&nbsp"+"&nbsp"+"&nbsp"+"&nbsp"+"&nbsp"+"&nbsp"+"&nbsp"+"&nbsp"+"&nbsp"+"&nbsp"+BRANCH+"&nbsp"+"&nbsp"+"&nbsp"+"&nbsp"+" "+YEAR+"&nbsp"+"&nbsp"+"&nbsp"+"&nbsp"+" "+SEM+"&nbsp"+"&nbsp"+"&nbsp"+"&nbsp"+" "+DIV+"&nbsp"+"&nbsp"+"&nbsp"+"&nbsp"+"&nbsp"+"&nbsp"+"&nbsp"+"&nbsp"+" "+SUB+"&nbsp"+"&nbsp"+"&nbsp"+"&nbsp"+" "+"Prof : "+PROFESSOR+"&nbsp"+"&nbsp"+"&nbsp"+"&nbsp"+"&nbsp"+"&nbsp"+"&nbsp"+"&nbsp";
